  ...
  <section class="cms-card p-5">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="m-0 mt-1 text-sm text-(--admin-muted)">Drag-and-drop upload and manage listing galleries, documents, and SEO images.</p>
      </div>
    </div>
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      <article v-for="item in cms.media" :key="item.id" class="rounded-lg border border-(--admin-border) bg-(--admin-soft) p-3">
        <div class="grid aspect-4/3 place-items-center rounded-md bg-(--admin-panel) text-2xl text-(--admin-primary)">
          <i class="pi pi-image" aria-hidden="true"></i>
        </div>
        <p class="m-0 mt-3 truncate text-sm font-medium">{{ item.name }}</p>
        <p class="m-0 mt-1 text-xs text-(--admin-muted)">{{ item.type }} · {{ item.size }}</p>
      </article>
    </div>
  </section>
</template>
